...
class Videos:
    ...
    def _load_initial_page(self):
        self._html, self._cookies = getInitPage(self._channel_id, 'channel_videos')
    ...
    def _extract_video_data_from_content(self, content):
        ...
        if videoGrid:
            video_data = {
                ...
                'length': self._get_length(videoGrid.get('lengthText').get('simpleText')),
                'views': int(re.sub(r"\D", "", videoGrid.get('viewCountText').get('simpleText'))),
                'description': videoGrid.get('descriptionSnippet', {}).get('runs', [])[0].get('text', {}) if videoGrid.get('descriptionSnippet', {}).get('runs', []) else None,
            }
            return video_data
        return None
